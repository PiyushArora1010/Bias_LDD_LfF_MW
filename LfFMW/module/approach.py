...
class Approach(nn.Module):
    ...
    def __init__(self, encoder_output_dim:int, output_dim:int, head: torch.nn.Module=None, classifier:torch.nn.Module=None,distance:str='cosine'):
        ...
        self.lay = nn.Softmax(dim = -1)
    


    def forward(self, encoder_output:torch.Tensor, memory_set:torch.Tensor, loss_weights, reweight, return_weights:bool=False)->torch.Tensor:
        ...
        if reweight:
            loss_weights = self.lay(loss_weights)
            tempMean = torch.mean(loss_weights)
            loss_weights[loss_weights < tempMean] = 0.0
            content_weights = content_weights * loss_weights 
